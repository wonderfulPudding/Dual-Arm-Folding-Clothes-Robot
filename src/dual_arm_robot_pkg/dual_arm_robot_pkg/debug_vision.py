    ...
    def run(self, duration=60.0):
        ...
        try:
            while (time.time() - start_time) < duration:
                ...
                print(f"Step {step}: Action Min={action_np.min():.3f}, Max={action_np.max():.3f}, Mean={action_np.mean():.3f}")
                ...
                cv2.putText(display_frame, "VISION CONTROL", (20, 100),
                          cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(display_frame, f"Action: {action_np.min():.2f}~{action_np.max():.2f}", 
                          (20, display_frame.shape[0] - 50),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                cv2.putText(display_frame, "Press 'Q' to STOP", (20, display_frame.shape[0] - 20),
